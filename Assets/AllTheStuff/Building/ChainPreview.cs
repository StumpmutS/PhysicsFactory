using UnityEngine;

public class ChainPreview : BuildingPreview
{
    public override void StretchTo(Vector3 origin, Vector3 destination, int cellSize)
    {
        var direction = destination - origin;
        transform.localScale = new Vector3(transform.localScale.x, transform.localScale.y, direction.magnitude + cellSize);
        transform.forward = direction;
        transform.position = Vector3.Lerp(destination, origin, .5f);
    }

    public override void Place()
    {
        var building = Instantiate(buildingPrefab, transform.position, transform.rotation);
        building.transform.localScale = transform.localScale;
    }
}